...
def checkout_pr(pr_number, owner="blender", repo="blender"):
    ...
    run_cmd(
        f"git pull --no-rebase --ff --no-edit --commit https://projects.blender.org/{pr_repo} {pr_branch}"
    )
...
def prune_branches():
    ...
    # Attempt to prune remote-tracking branches; ignore any errors/output
    run_cmd("git fetch --prune", check=False, capture_output=True)
...
""",
    ...
    parser.add_argument(
        "pr_number", nargs="?", help="Pull request number to fetch and checkout."
    )
    parser.add_argument(
        "--owner", "-o", default="blender", help="Repository owner (default: blender)."
    )
    parser.add_argument(
        "--repo", "-r", default="blender", help="Repository name (default: blender)."
    )
    parser.add_argument(
        "--prune",
        action="store_true",
        help="Prune local branches that have been merged or are stale.",
    )
